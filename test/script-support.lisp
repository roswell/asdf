...
(defun test-upgrade (old-method new-method tag) ;; called by run-test
  (with-test ()
    ...
    (format t "Testing it~%")
    (format t "UIOP: ~S ~S~%" (asymval :*uiop-version*) (acall :version-string (acall :system-version* (acall :find-system :uiop))))
    (format t "ASDF: ~S ~S~%" (get-asdf-version) (acall :version-string (acall :system-version* (acall :find-system :asdf))))
    (assert (equal (asymval :*uiop-version*) (acall :version-string (acall :system-version* (acall :find-system :uiop)))))
    (assert (equal (get-asdf-version) (acall :version-string (acall :system-version* (acall :find-system :asdf)))))
    (register-directory *test-directory*)
